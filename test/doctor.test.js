...
test('defaultProfileDir and defaultCacheDir honor XDG paths', () => {
  ...
  assert.equal(defaultProfileDir({ HOME: '/tmp/home' }), '/tmp/home/.config/Evernote');
  assert.equal(
...
test('redact removes common account and token identifiers', () => {
  ...
  assert.equal(redact(line), 'user <email> note <uuid> token="<redacted>" id <hex>');
});
...
test('file classifiers match Evernote profile files', () => {
  ...
  assert.equal(
    isLogFile('/home/user/.config/Evernote/Partitions/x/Local Storage/leveldb/000003.log'),
    false,
  );
...
test('isEvernoteProcessLine avoids matching unrelated paths', () => {
  assert.equal(
    isEvernoteProcessLine(
      ' 123 00:01:02 Evernote /tmp/.mount_Evernote/usr/lib/evernote/Evernote --no-sandbox',
    ),
    true,
  );
  assert.equal(
