...
const EVERNOTE_URL =
  process.env.EVERNOTE_URL || 'https://win.desktop.evernote.com/builds/Evernote-latest.exe';
...
const nativeModules = [
  ...
  },
  {
    ...
    target: 'resources/app.asar.unpacked/node_modules/@ronomon/opened/binding.node',
  },
  {
    name: 'electron-native-auth',
    packagePath: 'node_modules/electron-native-auth/package.json',
    source: 'node_modules/electron-native-auth/build/Release/electron_native_auth.node',
    target:
...
function build() {
  ...
  run('7z', ['x', '-y', path.join(nsisDir, '$PLUGINSDIR', 'app-64.7z'), `-o${payloadDir}`]);
  ...
  const electronZip = path.join(CACHE_DIR, `electron-v${electronVersion}-linux-${TARGET_ARCH}.zip`);
