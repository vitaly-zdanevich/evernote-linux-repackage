...
function redact(value) {
  return String(value)
    ...
    .replace(/((?:auth|access|refresh)?token|password|secret)(["']?\s*[:=]\s*)["'][^"',\s]+["']/gi, "$1$2\"<redacted>\"");
}
...
function doctor(options) {
  ...
  if (busctl) {
    ...
    printCheck(
      ...
      result.status === 0,
      result.status === 0 ? "available" : redact((result.stderr || result.stdout).trim() || "not available"),
    );
  } else {
    printCheck("busctl", false, "not installed; cannot check Secret Service over DBus");
  }
  printCheck("secret-tool", Boolean(commandPath("secret-tool")), "optional command-line keyring test tool");
  ...
  process.stdout.write(`Indexed files: ${profileFiles.length}${profileFiles.length >= MAX_WALK_FILES ? " (limit reached)" : ""}\n`);
  ...
  if (logFiles.length === 0) {
    ...
    if (matches.length > 0) {
      process.stdout.write("Review before sharing; log lines may still contain account or note metadata.\n");
